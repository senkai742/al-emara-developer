...
<head>
    ...
    <title>Forgot Password - Al Emara Developer</title>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="mb-8 lg:text-left text-center">
                <div class="w-12 h-12 bg-[#3eb27e] bg-opacity-10 rounded-xl flex items-center justify-center mb-4 lg:mx-0 mx-auto">
                    <i data-lucide="key-round" class="w-6 h-6 text-[#3eb27e]"></i>
                </div>
            ...
            @if(session('debug'))
                <div class="mb-6 p-4 bg-gray-100 border border-gray-300 rounded-lg">
                    <p class="text-xs font-mono text-gray-700 break-all">{{ session('debug') }}</p>
                </div>
            @endif
            ...
            <form action="{{ route('password.email') }}" method="POST" class="space-y-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email') }}" class="block w-full rounded-lg border border-gray-200 pl-11 pr-4 py-3 text-sm focus:border-[#3eb27e] focus:ring-1 focus:ring-[#3eb27e] transition-colors bg-gray-50 focus:bg-white placeholder-gray-400" placeholder="Enter your email" required autofocus>
                    </div>
                </div>
            ...
            <div class="mt-8 text-center text-sm">
                <p class="text-gray-500">Remember your password? <a href="{{ route('login') }}" class="text-[#3eb27e] hover:text-[#2a7a56] font-medium">Sign in</a></p>
            </div>
